...
export const EditImage = defineComponent({
  ...
  setup(props) {
    ...
    const ltOverlap = (x, y) => x < y;
    const rbOverlap = (x, y) => x > y;
    const updateCursor = (e) => {
      ...
      const { x, y } = e;
      if (
        (ltOverlap(x, left) && ltOverlap(y, top)) ||
        (rbOverlap(x, right) && rbOverlap(y, bottom))
      ) {
        ...
        (rbOverlap(x, right) && ltOverlap(y, top)) ||
        (ltOverlap(x, left) && rbOverlap(y, bottom))
      ) {
    ...
    function updateMaskStyle(rect, parentRect) {
    ...
    function observerImage() {
      ...
      const resizeObserver = useResizeObserver(image, ([entry]) => {
        ...
        nextTick(() => {
          ...
          const left =
            (Number(cutInfo.value.left.replace("px", "")) * newWidth) /
            oldWidth;
          const top =
            (Number(cutInfo.value.top.replace("px", "")) * newHeight) /
            oldHeight;
          cutInfo.value.left = left + "px";
          cutInfo.value.top = top + "px";
          if (!cutInfo.value.width.match("%")) {
            const width =
              (Number(cutInfo.value.width.replace("px", "")) * newWidth) /
              oldWidth;
            cutInfo.value.width = width + "px";
          }
          if (!cutInfo.value.height.match("%")) {
            const height =
              (Number(cutInfo.value.height.replace("px", "")) * newHeight) /
              oldHeight;
            cutInfo.value.height = height + "px";
          }
          const parent = region.value?.parentElement;
    ...
    const lineColorRef = computed(() =>
      graphState.value === "markerpen"
        ? colorToHalfTransparent(lineColor.value, 0.4)
        : lineColor.value,
    );
    ...
    const graphStateToggle = (e, options) => {
      e.stopPropagation();
      lineWidth.value = options.lineWidth;
      graphState.value =
        graphState.value === options.state ? null : options.state;
    ...
    const originalImageRect = ref(null);
    watch(
      () => img.value,
      async () => {
    ...
    const save = async (e) => {
      ...
      if (canvasInfo.value.graphList)
        canvasInfo.value.graphList.forEach((list) =>
          setCanvas(canvas, list, { ...scaleInfo, isSave: true }),
        );
      ...
      if (region.value) {
        ...
        const left =
          Number(cutInfo.value.left.replace("px", "")) * scaleInfo.wScale;
        const top =
          Number(cutInfo.value.top.replace("px", "")) * scaleInfo.hScale;
    ...
    const from = computed(() => {
      if (props.from) {
        ...
        from.left = Math.min(
          from.left - (clientWidth - Math.min(from.width, parentMaxWidth)) / 2,
          from.left,
        );
        from.top = Math.min(
          from.top -
            (clientHeight - Math.min(from.height, parentMaxHeight)) / 2,
          from.top,
        );
     ...
     * @returns
     */
    function setCanvas(canvas, lineInfo, scaleInfo) {
      if (!canvas) return;
      ...
      if (key) {
        ...
        if (
          lineInfoMap[key] &&
          JSON.stringify(lineInfoMap[key]) === JSON.stringify(lineInfo)
        ) {
          return;
        } else {
          // 数据更新重新保存
      ...
      if (lineInfo.type === "rect") {
        ...
        if (!scaleInfo) {
          const { width: imgWidth, height: imgHeight } =
            image.value.getBoundingClientRect();
          const { width: primitiveW, height: primitiveH } =
            originalImageRect.value;
          option.scaleInfo = {
            wScale: primitiveW / imgWidth,
            hScale: primitiveH / imgHeight,
          };
